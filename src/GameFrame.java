...
public class GameFrame extends MyFrame{
	public void run() {
		GameWorld.player=new Player(100,300,0,0,1,1);
		...
		while(true) {
			...
			GameWorld.enemies.add(new EnemyBase(100,50,GameWorld.stage,0,20,20));
	...
	public void checkPlayerAndEnemies() {
		for(int i=0;i<GameWorld.enemies.size();i++) {
			...
			if(checkHit(GameWorld.player,e)) {
				System.out.println("やられた！！うびびびびび");
	...
	public boolean checkHit(Character a,Character b) {
		if(Math.abs(a.x-b.x)<=a.hx+b.hx && Math.abs(a.y-b.y)<=a.hy+b.hy) {
			return true;
		}
		else {
			return false;
